"""Unified bridge supporting multiple IoT protocols with backpressure and reconnection."""
...
from opcua2uc.metrics import CONNECTED_SOURCES, EVENTS_DROPPED, EVENTS_INGESTED, EVENTS_SENT, QUEUE_DEPTH
from opcua2uc.protocols.base import ProtocolRecord
...
class UnifiedBridge:
    """Unified bridge supporting OPC-UA, MQTT, and Modbus protocols."""
    ...
    def __init__(self, cfg: AppConfig) -> None:
        ...
        self._source_clients: dict[str, Any] = {}
    ...
    async def start_source(self, name: str) -> None:
        """Start a source with automatic protocol detection."""
        name = name.strip()
        if not name:
        ...
        # Detect or use explicit protocol type
        protocol_type = src.get("protocol_type") or src.get("type")
        if not protocol_type:
            try:
                protocol_type = detect_protocol_type(endpoint)
            except ValueError:
                # Default to OPC-UA for backward compatibility
                protocol_type = "opcua"
        ...
        q: asyncio.Queue[ProtocolRecord] = asyncio.Queue(maxsize=max(1, queue_max))

        def on_stats(delta: dict[str, Any]) -> None:
            st = self._source_stats.setdefault(name, {})
            st.update(delta)

        def on_record(rec: ProtocolRecord) -> None:
            try:
                try:
                    q.put_nowait(rec)
        ...
        async def sender_loop() -> None:
            ...
            try:
                ...
                while not stop_evt.is_set():
                    ...
                    # Convert ProtocolRecord to dict for Zerobus
                    payload = rec.to_dict()
        ...
        async def task_main() -> None:
            ...
            try:
                ...
                protocol_config = {
                    **src,  # Include all source config
                    "reconnect_enabled": src.get("reconnect_enabled", True),
                    "reconnect_delay_ms": src.get("reconnect_delay_ms", 5000),
                    "reconnect_max_attempts": src.get("reconnect_max_attempts", 0),
                }
                ...
                self._source_clients[name] = client

                # Run protocol client with automatic reconnection
                protocol_task = asyncio.create_task(client.run_with_reconnect())

                await asyncio.wait([protocol_task, send_task], return_when=asyncio.FIRST_EXCEPTION)
                for t in (protocol_task, send_task):
                ...
                if name in self._source_clients:
                    try:
                        self._source_clients[name].request_stop()
                    except Exception:
                        pass
                    del self._source_clients[name]
                # Cancel tasks
                for t in (protocol_task, send_task):
                    if t and not t.done():
                        t.cancel()
    ...
    async def stop_source(self, name: str) -> None:
        ...
        self._source_tasks.pop(name, None)
        self._source_stop.pop(name, None)
        self._source_clients.pop(name, None)
        self._update_connected_sources()
